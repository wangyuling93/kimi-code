...
import { type Component, CURSOR_MARKER, compositeTuiLine, type MouseRegion, type RenderContext } from "./tui.ts";
...
interface LayoutContext {
	...
	/** Mouse hit-region collector for the current render pass, if any. */
	regions?: MouseRegion[];
}
...
function layoutComponent(
	...
	if (!node) {
		// Plain components render once at their final box position. Thread the
		// render context (with the box origin) so containers can register
		// mouse hit regions for their children. Scroll-view content and
		// measuring passes go through renderCached without a context.
		let lines: string[];
		if (context.regions && typeof component.render === "function") {
			const ctx: RenderContext = { row: y, col: x, regions: context.regions };
			lines = component.render(safeWidth, ctx);
		} else {
