...
export class TuiMainScreen extends TuiBase implements TUI {
	...
	protected override afterTerminalStart(): void {
		...
		this.addInputListener((data) => {
			const rest = this.consumeMouseInput(data);
			if (rest === null) return undefined;
			// Mouse sequences are extracted; the remainder (if any) continues
			// to the focused component. Do not set `consume` — that would stop
	...
	protected override getMouseViewportTop(): number {
