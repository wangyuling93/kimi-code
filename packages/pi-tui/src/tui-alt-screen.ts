...
export class TuiAltScreen extends TuiBase implements ViewportTUI {
	...
	private handleViewportInput(data: string): { consume?: boolean } | undefined {
		...
		if (mouseEvent) {
			...
			if (!handled && !this.dispatchAltScreenMouseEvent(mouseEvent)) this.handleSelectionMouseEvent(mouseEvent);
			return { consume: true };
		}
	...
	/**
	 * Route an SGR mouse event to a component hit region (e.g. the message-box
	 * editor). Returns true when a component consumed the event; otherwise the
	 * caller falls back to the screen-level selection handling.
	 */
	private dispatchAltScreenMouseEvent(event: SgrMouseEvent): boolean {
		const button = event.button & 3;
		const type = event.release ? "up" : (event.button & 32) !== 0 ? "drag" : "down";
		if (type === "drag" && this.mouseCapture === null) return false;
		const raw: RawMouseInput = {
			type,
			button,
			row: event.y,
			col: event.x,
		...
		// The alt screen has no scrolled buffer: regions are already in screen
		// coordinates, so the base dispatch needs no viewport offset.
		return this.dispatchMouseEvent(raw);
	}
	...
	protected override doRender(): void {
		...
		this.mouseRegions = [];
		let nextLayout = renderLayoutFrame(root, width, height, () => this.requestRender(), this.mouseRegions);
		if (this.refreshSearch(nextLayout)) {
			nextLayout = renderLayoutFrame(root, width, height, () => this.requestRender(), this.mouseRegions);
		}
		...
		if (cursorPos) {
			...
			if (this.getShowHardwareCursor()) {
				...
				if (this.cursorColor !== undefined) {
					buffer += `\x1b]12;${this.cursorColor}\x07`;
				}
				buffer += "\x1b[?25h";
			} else {
				buffer += "\x1b[?25l";
